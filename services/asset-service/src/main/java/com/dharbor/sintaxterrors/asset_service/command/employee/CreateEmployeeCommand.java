...
public class CreateEmployeeCommand
        extends SafeAbstractCommand<CreateEmployeeRequest, EmployeeResponse>
    ...
    public void preExecute() {
        log.info("CreateEmployeeCommand - PreExecute");
        employeeService.validateCreateEmployeeRequest(input);
    }

    @Override
    public void execute() {
        log.info("CreateEmployeeCommand - Execute");
        this.output = this.employeeService.saveEmployee(this.input);
    }

    @Override
    public void postExecute() {
        log.info("CreateEmployeeCommand - PostExecute");
        if (Utils.isNull(this.output)) {
            throw new ProcessErrorException(EmployeeExceptionConstants.FAILED_TO_CREATE_EMPLOYEE);
        }
    }
}