...
public class GetCurrentAssignedAssetsCommand
        extends SafeAbstractCommand<Long, List<AssetResponse>>
    ...
    @Override
    public void execute() {
        log.info("GetCurrentAssignedAssetsCommand - Execute");
        this.output = transactionService.getCurrentAssignedAssets(this.input);
    }

    @Override
    public void postExecute() {
        log.info("GetCurrentAssignedAssetsCommand - PostExecute");
        if (Utils.isNull(this.output)) {
            throw new ProcessErrorException(TransactionExceptionConstants.FAILED_TO_GET_ASSIGNED_ASSETS);
        }
    }
}
