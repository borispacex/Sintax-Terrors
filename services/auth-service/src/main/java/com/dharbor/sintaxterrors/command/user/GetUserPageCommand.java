...
public class GetUserPageCommand
        extends SafeAbstractCommand<GetUserPageRequest, PaginationResponse<UserResponse>>
    ...
    @Override
    public void execute() {
        log.info("GetAllUserCommand - Execute");
        this.output = userService.getUserPage(this.input);
    }

    @Override
    public void postExecute() {
        log.info("GetAllUserCommand - PostExecute");
        if (Utils.isNull(this.output)) {
            throw new ProcessErrorException(UserExceptionConstant.FAILED_TO_GET_PAGE_USER_MESSAGE);
        }
    }
