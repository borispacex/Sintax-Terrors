...
export class CreateEmployeeCmd
  implements FormatCommand<CommonResponse<Employee>>
{
  ...
  execute(
    ...
    return this._httpClient
      .post<CommonResponse<Employee>>(this._urlBuilder(), request)
      .pipe(
        tap(() => {
  ...
  private _urlBuilder(): string {
    return API_PATHS.employee.create.generate();
  }

  private _errorHandler(error: any): Observable<never> {
    this._toastService.show({
      type: 'error',
      message: 'An error has occurred while creating employee.',
    });
