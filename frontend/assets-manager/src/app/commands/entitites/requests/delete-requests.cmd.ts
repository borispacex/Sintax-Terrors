...
export class DeleteRequestsCmd
  implements FormatCommand<CommonResponse<Request>>
{
  ...
  execute(id: number): Observable<CommonResponse<Request>> {
    return this._httpClient
      .delete<CommonResponse<Request>>(this._urlBuilder(id))
      .pipe(
        tap(() => {
  ...
  private _urlBuilder(id: number): string {
    return API_PATHS.request.delete.generate({ id });
  }

  private _errorHandler(error: any): Observable<never> {
    this._toastService.show({
      type: 'error',
      message: 'An error has occurred while deleting request.',
    });
