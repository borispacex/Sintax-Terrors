...
export class CreateRequestsCmd
  implements FormatCommand<CommonResponse<Request>>
{
  ...
  execute(request: CreateRequestRequest): Observable<CommonResponse<Request>> {
    return this._httpClient
      .post<CommonResponse<Request>>(this._urlBuilder(), request)
      .pipe(
        tap(() => {
  ...
  private _urlBuilder(): string {
    return API_PATHS.request.create.generate();
  }

  private _errorHandler(error: any): Observable<never> {
    this._toastService.show({
      type: 'error',
      message: 'An error has occurred while creating request.',
    });
